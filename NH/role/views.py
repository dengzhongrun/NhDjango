...
def jx_list(request):
    ...
    # 构造搜索
    data_dict = {}
    search_data = request.GET.get('j', "")
    if search_data:
        data_dict["recept_staff"] = search_data

    # 根据搜索条件去数据库获取
    queryset = models.PerformanceTask.objects.filter(**data_dict)
    # 分页
    ...
    return render(request, 'jixiao.html', context)
